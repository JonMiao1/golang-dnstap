...
var (
        ...
        //Temporary flag. Later will take in stream info as argument maybe, currently hard code the stream.
        //flagKinesis    = flag.Bool("k", false, "write to kinesis")
)
...
func main() {
        ...
        flag.StringVar(&streamname, "k", "", "write dnstap payloads to kinesis stream name, requires a text format flag")
        flag.StringVar(&region, "region", "", "optional AWS region for kinesis output")
        ...
        if *flagWriteFile != "" || len(tcpOutputs)+len(unixOutputs) == 0 || streamname != "" {
                ...
                //Output to Kinesis
                if streamname != "" {
                        o, err := dnstap.NewKinesisOutput(streamname, region, format)
                        if err != nil {
                                fmt.Fprintf(os.Stderr, "dnstap: Kinesis output error: %v\n", err)
                                os.Exit(1)
                        }
                        go o.RunOutputLoop()
                        output.Add(o)
